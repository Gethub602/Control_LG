...
SCENARIO = "seq_70_95_90_73"
PREFIX = "dynamic_unseen"
METHODS = ["db", "rf", "dl"]
COLORS = {"db": "#4C78A8", "rf": "#F58518", "dl": "#B279A2"}
    ...
    for method in METHODS:
        paths = sorted(
            KAFKA_DIR.glob(
                ...
                f"{PREFIX}_{method}_{SCENARIO}_rep*.csv"
            ),
...
def make_timeseries(df, timestamp):
    ...
    for ax in axes:
        for change_time in [5, 10, 15]:
            ax.axvline(change_time, color="0.25", linestyle=":", linewidth=1)

    axes[-1].set_xlabel("Time [s]")
    fig.suptitle("Dynamic unseen target scenario: 70 -> 95 -> 90 -> 73 RPM", fontsize=15)
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    save_path = FIGURE_DIR / f"dynamic_unseen_db_rf_dl_timeseries_{timestamp}.png"
...
def make_gain_distribution(df, timestamp):
    ...
    csv_path = SUMMARY_DIR / f"dynamic_unseen_gain_distribution_{timestamp}.csv"
    ...
    fig.suptitle("Dynamic unseen scheduled gain distribution", fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.92])
    fig_path = FIGURE_DIR / f"dynamic_unseen_gain_distribution_{timestamp}.png"
